...
<img src="assets/branding/app\_icon.png" width="110" alt="F1 Vision logo" />
...
[!\[Style](https://img.shields.io/badge/style-flutter\_\_lints-1E88E5?style=flat-square)](https://pub.dev/packages/flutter_lints)
...
<sub>*Analytics Command Center — fourteen live panels, nine chart types, one screen.*</sub>
...
An engineer-style cockpit streaming OpenF1 `/car\_data`.
...
one class and registering it in `core\_providers.dart`. **No model, repository or
...
**Colour** — `lib/core/theme/app\_colors.dart`. Surfaces and text are
...
|Charts|**fl\_chart 0.69**|Covers line, bar, pie, radar and scatter; custom painters fill the rest|
...
|Motion|**flutter\_animate**, **shimmer**|Declarative entrances and skeletons|
|Type|**google\_fonts**|Sora + Inter without bundling binaries|
...
flutter create . --project-name f1\_vision \\
...
dart run flutter\_native\_splash:create
```
...
```
lib/core/constants/app\_info.dart
```
...
[**docs/FIREBASE\_SETUP.md**](docs/FIREBASE_SETUP.md):
...
`google-services.json`, `GoogleService-Info.plist` and `firebase\_options.dart`
...
**Web.** `path\_provider` has no web implementation, so Isar initialisation fails
...
[**docs/WINDOWS\_BUILD.md**](docs/WINDOWS_BUILD.md). Google Sign-In has no
...
Deeper detail in [**docs/OFFLINE\_CACHE.md**](docs/OFFLINE_CACHE.md).
...
│   ├── live\_race/   replay/      settings/   shell/
│   ├── standings/   telemetry/
...
|-|-|
|`Target of URI hasn't been generated: cached\_response.g.dart`|Isar codegen not run → `dart run build\_runner build --delete-conflicting-outputs`|
|Windows: `Compatibility with CMake < 3.5 has been removed`|Old Firebase C++ SDK vs CMake 4 → see [docs/WINDOWS\_BUILD.md](docs/WINDOWS_BUILD.md)|
...
* [**fl\_chart**](https://github.com/imaNNeo/fl_chart) — doing the heavy lifting
