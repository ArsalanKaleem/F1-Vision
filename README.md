...
**Recording your own captures.** Drop GIFs into `docs/media/` using the
filenames above. On desktop, record with any screen recorder and convert:

```bash
ffmpeg -i input.mov -vf "fps=15,scale=880:-1:flags=lanczos" -loop 0 docs/media/hero.gif
```

On mobile, use `scrcpy --record` or your device recorder. Keep each clip under
roughly 6 MB so GitHub renders it inline rather than linking it.
