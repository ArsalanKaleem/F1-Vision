...
- **Driver Comparison Studio** — pick any two drivers and compare them across
  the season and inside a single race: profile overview, head-to-head metric
  bars, performance radar, championship progress, lap times, sectors, speed,
  position changes, tyre strategy, pit stops and a pace-vs-tyre-age
  degradation scatter.
- **Race Replay & Strategy Simulator** *(signature feature)* — replay any
  completed Grand Prix lap by lap (back to 1996): animated leaderboard, tyre
  strategy, race timeline, position-history / lap-time / speed charts, a
  synchronized race feed and playback controls (0.5×–5×, jump-to-lap). Built on
  a pluggable telemetry-provider seam so new data sources drop in without UI
  changes.
...
### Required once: generate the offline-cache schema
The Isar cache uses generated code, so run this after `flutter pub get`:

```bash
dart run build_runner build --delete-conflicting-outputs
```

See **[docs/OFFLINE_CACHE.md](docs/OFFLINE_CACHE.md)** for how the two-tier
cache works. If Isar can't open on your platform the app falls back to an
...
In the Replay Studio: `Space` play/pause · `←` `→` step lap · `R` restart ·
`Home` / `End` jump to start or finish · `1`–`4` replay speed.
